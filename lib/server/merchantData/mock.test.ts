...
async function main() {
  ...
  await test("itemLabels: menu items, coupons and whatsNew resolve too; section searched first", async () => {
    ...
    const other = await data.createRequest(
      merchant,
      DOOBIE,
      { section: "other", action: "update", itemIds: ["coupon-doobie-bogo-cart", "dn-news-1"], message: "x" },
      []
    );
    assert.deepEqual(other.itemLabels, ["Buy one cart, get the other for $1", "Order ahead, skip the line"]);
  ...
  await test("listSummaries counts open requests per merchant", async () => {
    ...
    assert.equal(list.find((m) => m.id === "store-template")?.openRequests, 0);
  });
