...
import type { Card, OfferCard, ResolvedCards } from "./types";
...
/** The one pool the set uses. Its FIRST code goes to the first sid that asks: 7e57ca4d00000007. */
...
function resolveCode(spec: CodeSpec, sid: string): { value: string; scope: "shared" | "unique" } {
  switch (spec.mode) {
    ...
    case "template":
      return {
        value: spec.template.replace("{sid6}", sid.slice(-6).toUpperCase()).replace("{sid8}", sid.slice(-8).toUpperCase()),
        scope: "unique",
      };
    case "perSid":
...
function buildConfig(merchantId: MerchantId) {
  ...
  return {
    ...
    merchant: {
      ...
      ...(m.licenseLine ? { licenseLine: m.licenseLine } : {}),
    },
    layout: {
      order: "receipt-first",
      ...(cfOverrides.length ? { overrides: [{ when: { fact: "sid", op: "in", value: cfOverrides }, order: "cards-first" }] } : {}),
    },
    links: { ctaHostAllowlist: ["papex.app"] },
