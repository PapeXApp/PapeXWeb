...
const defs = h(
  ...
  lin("rdhTopFill", [0, 1, 1, 0], [[0, "#3A3D42"], [0.5, "#2A2D31"], [1, "#1F2124"]]),
  lin("rdhSideFill", [0, 0, 0, 1], [[0, "#2B2E33"], [0.55, "#1E2023"], [1, "#151618"]]),
  lin("rdhSideSweep", [0, 0, 1, 0], [[0, "#FFFFFF", 0.035], [0.6, "#FFFFFF", 0], [1, "#000000", 0.12]]),
  lin("rdhEndFill", [0, 0, 0, 1], [[0, "#1D1F22"], [0.6, "#141517"], [1, "#0D0E0F"]]),
  lin("rdhEdgeDown", [0, 0, 0, 1], [[0, RIM, 0.2], [1, RIM, 0]]),
  lin("rdhEdgeUp", [0, 1, 0, 0], [[0, RIM, 0.14], [1, RIM, 0]]),
  lin("rdhEdgeLeft", [0, 0, 1, 0], [[0, RIM, 0.13], [1, RIM, 0]]),
  lin("rdhEdgeRight", [1, 0, 0, 0], [[0, RIM, 0.14], [1, RIM, 0]]),
  h(
    "pattern",
    { id: "rdhLayers", width: 8, height: 1.3, patternUnits: "userSpaceOnUse" },
    h("rect", { width: 8, height: 0.3, fill: "#FFFFFF", fillOpacity: 0.028 }),
    h("rect", { y: 0.66, width: 8, height: 0.24, fill: "#000000", fillOpacity: 0.1 })
  ),
  h(
    "pattern",
    { id: "rdhHatch", width: 1.25, height: 1.25, patternUnits: "userSpaceOnUse", patternTransform: "rotate(45)" },
    h("rect", { width: 0.3, height: 1.25, fill: "#FFFFFF", fillOpacity: 0.022 })
  ),
  lin("rdhSticker", [0, 0, 1, 1], [[0, "#F8F8F5"], [0.6, "#F3F3F0"], [1, "#E8E8E4"]]),
  rad("rdhLedHalo", [[0, LED, 0.7], [0.35, LED, 0.28], [1, LED, 0]]),
  rad("rdhLedGlow", [[0, LED, 0.5], [1, LED, 0]]),
  rad("rdhLedDome", [[0, "#F4FFF8"], [0.28, "#B4F9D1"], [0.62, LED], [1, "#16924D"]], { cx: 0.4, cy: 0.36, r: 0.7 }),
  lin(
    "rdhCableFade",
    [fx(fadeFrom[0]), fx(fadeFrom[1]), fx(fadeTo[0]), fx(fadeTo[1])],
    [[0, "#000000"], [0.32, "#FFFFFF"], [0.85, "#FFFFFF"], [1, "#000000"]],
    { gradientUnits: "userSpaceOnUse" }
  ),
  h(
    "mask",
    { id: "rdhCableMask", maskUnits: "userSpaceOnUse", x: 0, y: 0, width: VB_W, height: VB_H },
    h("rect", { width: VB_W, height: VB_H, fill: "url(#rdhCableFade)" })
  ),
  h("clipPath", { id: "rdhBodyClip" }, h("path", { d: roundedPoly(SILHOUETTE, 1.5) }))
);
...
const led = [
  ...
  h("circle", { cx: fx(ledScreen[0]), cy: fx(ledScreen[1]), r: 4.2, fill: "url(#rdhLedGlow)" }),
  h("ellipse", { cx: fx(ledScreen[0]), cy: fx(ledScreen[1]), rx: 2.15, ry: 1.75, fill: "url(#rdhLedDome)" }),
  h("ellipse", {
    cx: fx(ledScreen[0] - 0.55),
    cy: fx(ledScreen[1] - 0.55),
    rx: 0.6,
    ry: 0.42,
    fill: "#FFFFFF",
    fillOpacity: 0.9,
...
const scene = [defs, faces, sticker, ...led, ...cable];

/* ---------------------------------------------------------------- output -- */
const svgOut = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${VB_W} ${VB_H}" role="img" aria-label="${ARIA}">
...
${scene.map((n) => ser(n, "svg", "  ")).filter((s) => s.trim()).join("\n")}
</svg>
`;
...
const all = [...SILHOUETTE, ...cab];
const xs = all.map((p) => p[0]);
const ys = all.map((p) => p[1]);
console.log(`viewBox 0 0 ${VB_W} ${VB_H}`);
console.log(`art bounds x ${fx(Math.min(...xs))}..${fx(Math.max(...xs))}  y ${fx(Math.min(...ys))}..${fx(Math.max(...ys))}`);
console.log(`LED centre ${fx(ledScreen[0])},${fx(ledScreen[1])}`);
