...
- `npm run lint` has **15 pre-existing failing files** (recounted 2026-09-24): `app/dashboard/components/{BlogEditor,BlogList,Sidebar}.tsx`, `app/global-error.tsx`, `app/pos-calculator/pos-calculator-client.tsx`, `components/{AdminLogin,CreateBlogModal,EditBlogModal}.tsx`, `components/framer/anim/{FlightRoute,PlaneFlight}.tsx`, `components/ui/{calendar.tsx,use-toast.ts}`, `lib/{blogService,blogServiceFree,imageUpload}.ts`. A 16th means you added one. They predate the redesign. Verified 2026-09-09: nothing under `components/brand`, `components/paths`, `app/r`, `app/merchant`, `app/business` or `app/customers` contributes an error — keep it that way, and don't read the red output as "I broke it".
...
- `npm test` runs the unit tests (tsx, `lib/**/*.test.ts` + a few `app/r` render tests: ESC/POS parity, receipt state, merchant host routing, signup). They are not in CI; `npm test` + `npm run build` + a real browser pass is the gate. First build on a fresh machine needs network (Inter via `next/font/google`).

## Route map

29 `page.tsx` routes. The redesign ones first:
...
- `/about`, `/survey`, `/privacy`, `/terms`, `/pci`, `/support`, `/app-support`.
- Temporary (307) redirects in `next.config.ts` (Nico, 2026-09-24): `/waitlist` → `/` (the page is deleted), `/contact` → `/about` (no page), `/pos-calculator` → `/business` (page still in the repo).
...
- Client Firebase is project `papexweb-aed97` — config is HARDCODED in `firebase/firebaseConfig.ts` (the `NEXT_PUBLIC_FIREBASE_*` lines in `.env.example` are commented out and never read). Collections: `blogs`, `waitlist`, `blog_subscribers`. Firestore security rules are NOT in this repo (console-managed).
- Firebase is ALSO server-side now: `/api/signup` uses firebase-admin with env `PAPEXWEB_SERVICE_ACCOUNT` (same `papexweb-aed97` project) and emails via AWS SES v2 (`AWS_SES_*`, `SIGNUP_NOTIFY_*`); until that key is set the demo form falls back to its old client write. The merchant API routes use a second admin app on `papexv2` (`PAPEXV2_SERVICE_ACCOUNT`). See `docs/SIGNUP_ROUTE.md`.
...
- **Two coexisting design systems, separated by a scope class.** `.rd` activates `styles/papex-brand.css` (the redesign tokens); the legacy pages use `.framer-site` / `styles/framer-site.css`. Everything brand-new must sit inside `.rd`; a legacy page body must not.
  ...
  - `components/framer/framer-page-shell.tsx` (the re-skinned legacy pages — seven routes: `/app-support`, `/pci`, `/pos-calculator` (redirected, see above), `/privacy`, `/support`, `/survey`, `/terms`) gives `SiteNav` and `SiteFooter` **their own separate `.rd` wrappers and deliberately leaves `<main className="framer-subpage">` OUTSIDE**. That scoping is load-bearing — collapsing it into one wrapper restyles the legacy page bodies.
...
Its whole subtree is unreachable with it: `components/framer/framer-nav.tsx` is imported only by `framer-landing-page.tsx`, and `public/framer-assets/logo.svg` is referenced only by `framer-nav.tsx`. **So the old letterboxed 1920×1080 wordmark does not render anywhere on the live site** — the legacy routes get the new `SiteNav` via `FramerPageShell`. Verified 2026-09-09: the served HTML for `/contact`, `/privacy` and `/pos-calculator` contains zero references to it. Don't spend time tightening that SVG's viewBox to fix a nav logo; you'd be editing an asset nothing displays.
