...
- `npm run lint` has **16 pre-existing failing files** (`lib/blogService*`, `lib/imageUpload*`, `components/ui/*`, `components/{AdminLogin,CreateBlogModal,EditBlogModal}`, `components/framer/anim/*`, `app/dashboard/components/*`, `app/blog/[slug]`, `app/global-error`, `app/pos-calculator/pos-calculator-client`; recounted 2026-09-24 after the waitlist route and forms were deleted). They predate the redesign. Verified 2026-09-09: nothing under `components/brand`, `components/paths`, `app/r`, `app/merchant`, `app/business` or `app/customers` contributes an error — keep it that way, and don't read the red output as "I broke it".
